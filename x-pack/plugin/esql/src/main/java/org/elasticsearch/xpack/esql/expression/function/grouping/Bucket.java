...
public class Bucket extends GroupingFunction.EvaluatableGroupingFunction
    ...
    private record DateRoundingPicker(int buckets, long from, long to, ZoneId zoneId, long offset) {
        ...
        static final Unit FINAL_FALLBACK_UNIT = Unit.of(Rounding.DateTimeUnit.YEAR_OF_CENTURY);
        static final Unit TERTIARY_UNIT = Unit.of(Rounding.DateTimeUnit.MONTH_OF_YEAR);
        static final Unit SECONDARY_UNIT = Unit.of(Rounding.DateTimeUnit.WEEK_OF_WEEKYEAR);
        ...
        Rounding pickRounding() {
            ...
            if (roundingIsOk(SECONDARY_UNIT.rounding(zoneId))) {
                return SECONDARY_UNIT.rounding(zoneId);
            }
            if (roundingIsOk(TERTIARY_UNIT.rounding(zoneId))) {
                return TERTIARY_UNIT.rounding(zoneId);
            }
            return FINAL_FALLBACK_UNIT.rounding(zoneId);
        }
